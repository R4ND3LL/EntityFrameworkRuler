...
function Test-LastExitCode {
    ...
    if ($LASTEXITCODE -ne 0) {
        Write-Host "`n[FAIL] $Step" -ForegroundColor Red
        Write-Host "Exit code: $LASTEXITCODE" -ForegroundColor Red
        exit $LASTEXITCODE
    }
    Write-Host "[OK] $Step`n" -ForegroundColor Green
}

Write-Host "Local CI Simulation for EntityFrameworkRuler" -ForegroundColor Yellow

# Step 1: Restore dependencies
Write-Step "Restoring dependencies"
dotnet restore EntityFrameworkRuler.CI.slnf --v:m
Test-LastExitCode "dotnet restore"

# Step 2: Test Build (Debug)
Write-Step "Building in Debug configuration"
dotnet build EntityFrameworkRuler.CI.slnf --configuration Debug --no-restore --v:q
...
if (-not $SkipClean) {
    ...
    if (Test-Path "./nupkg") {
        ...
        Write-Host "[OK] Removed nupkg folder`n" -ForegroundColor Green
    } else {
        Write-Host "[INFO] nupkg folder does not exist, skipping`n" -ForegroundColor Yellow
    }
}

# Step 6: Release Build
Write-Step "Building in Release configuration"
dotnet build ./EntityFrameworkRuler.CI.slnf --configuration Release --no-restore --v:q
Test-LastExitCode "dotnet build (Release)"

# Step 7: Create NuGet packages
if (-not $SkipPack) {
    Write-Step "Creating NuGet packages"
    dotnet pack --configuration Release --no-build --no-restore ./EntityFrameworkRuler.CI.slnf --property:PackageOutputPath="$PWD\nupkg"
...
Write-Host "`nALL CI STEPS PASSED." -ForegroundColor Green
Write-Host "Your changes are ready to push to GitHub." -ForegroundColor Green
