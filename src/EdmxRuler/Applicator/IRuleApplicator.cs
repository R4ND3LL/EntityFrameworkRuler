...
public interface IRuleApplicator : IRuleProcessor {
    ...
    /// <returns></returns>
    Task<ApplyRulesResponse> ApplyRules(NavigationNamingRules navigationNamingRules,
    ...
    /// <param name="modelsFolder"> Optional folder where models are found. If provided, only cs files in the target subfolders will be loaded. </param>
    /// <returns></returns>
    Task<ApplyRulesResponse> ApplyRules(PrimitiveNamingRules primitiveNamingRules,
        string contextFolder = null, string modelsFolder = null);
